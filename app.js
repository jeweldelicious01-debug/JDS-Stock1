...
window.stockApp = function() {
    return {
        ...
        suppliers: [], 
        purchaseOrders: [], 
        events: [], // Holds live event alert rows streamed down from Firestore
        ...
        // Form object context tracking for scheduling complex high-pax catered events
        ...
        showEventAdminModal: false, // Toggles specialized banquet alert management panel view
        ...
        async init() {
            ...
            onSnapshot(colRef('events'), (snap) => { this.events = snap.docs.map((d) => ({ id: d.id, ...d.data() })); });
        ...
        /**
         * 🟢 AUTOMATED VANISHING EVENTS FILTER GETTER
         * Parses and evaluates event date codes dynamically against local machine calendar clocks.
         * If the event scheduled date is older than today, it automatically filters it out of view.
         */
        get activeLiveEvents() {
            const clientDateString = new Date().toISOString().slice(0, 10); // Outputs clean target comparison map 'YYYY-MM-DD'
            return this.events.filter(event => {
                return event.date >= clientDateString; // Only true if event is today or scheduled for the future
        ...
        /**
         * 🟢 NEW MULTI-EVENT WRITER METHOD
         * Provisions highly structured caterer allocation rows straight to the cloud events repository schema.
         */
        async submitNewEvent() {
            const { title, date, time, pax, targetItems } = this.formEvent;
            if (!title.trim() || !date || !time || !pax) {
                return alert("Title, Date, Time, and Pax fields are strictly mandatory parameters.");
            }
            try {
                await addDoc(colRef('events'), {
                    title: title.trim(),
                    date: date, // Saved clean as standard picker date maps: 'YYYY-MM-DD'
                    time: time.trim(),
                    pax: parseInt(pax) || 0,
                ...
                alert("🎯 Dynamic banquet alert distributed successfully to all network dashboard overlays!");
        ...
        /**
         * 🟢 MULTI-EVENT REMOVER DELETER AXIS
         * Direct manual override deletion command handler allowing administrators to strip an active notice row early.
         */
        async purgeEventAlert(eventId) {
            if (confirm("Permanently purge this event alert from the shared system grid tables?")) {
