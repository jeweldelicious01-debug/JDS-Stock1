...
function parseQuantityInput(inputStr, itemName = "") {
    ...
    const nameHasKgOrG = /\b(kg|kgs|kilo|kilograms?|g|gm|gms|grams?)\b/i.test(itemName) || ['kg', 'g'].includes(target);
    const nameHasLOrMl = /\b(l|ltr|liters?|litres?|ml|milliliters?|millilitres?)\b/i.test(itemName) || ['L', 'ml'].includes(target);

    // Rule: Plain integer inputs with 2 digits -> kg / L; 3 or more digits -> g / ml
    if (/^\d+$/.test(str)) {
        const val = parseInt(str, 10);
        if (nameHasKgOrG) {
            if (str.length <= 2) {
                // 1 or 2 digits = KG
                return target === 'g' ? val * 1000 : val;
            } else {
                // 3 or more digits = Grams
                return target === 'kg' ? Math.round((val / 1000) * 1000) / 1000 : val;
            }
        } else if (nameHasLOrMl) {
            if (str.length <= 2) {
                // 1 or 2 digits = Litres
                return target === 'ml' ? val * 1000 : val;
            } else {
                // 3 or more digits = ML
                return target === 'L' ? Math.round((val / 1000) * 1000) / 1000 : val;
            }
        }
    }
...
export function stockApp() {
    return {
        ...
        addItemToOrder() {
            if (!this.orderDesk.selectedItemId || !this.orderDesk.selectedQty) {
            ...
            const parsedQty = parseQuantityInput(this.orderDesk.selectedQty, itemObj.name);
        ...
        addReconcileExtraItem() {
            ...
            const parsedExtra = parseQuantityInput(this.reconcileExtraQty, target.name);
            if (isNaN(parsedExtra) || parsedExtra <= 0) {
                return alert("Please enter a valid extra quantity.");
            }
        ...
        async modifyThreshold(item) {
            let promptVal = prompt('Update safety limit (e.g. 10 for kg/L or 500 for g/ml):', formatShortQty(item.threshold, item.name));
            if (promptVal !== null) {
